...
try {
  ...
  // 1) Write shared version file for frontend to consume
  const jsDir = path.join(__dirname, 'public', 'js');
  if (!fs.existsSync(jsDir)) fs.mkdirSync(jsDir, { recursive: true });
  const versionFile = path.join(jsDir, 'version.js');
  const versionContents = `// Auto-generated by add-versioning.js\nwindow.__APP_VERSION = ${timestamp};\n`;
  fs.writeFileSync(versionFile, versionContents, 'utf8');
  console.log(`📝 Wrote version file: public/js/version.js (v=${timestamp})`);

  // 2) Process HTML files to add ?v=timestamp for local assets
  processHtmlFiles();
